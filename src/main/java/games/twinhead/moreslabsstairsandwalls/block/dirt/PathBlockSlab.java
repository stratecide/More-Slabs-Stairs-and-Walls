...
public class PathBlockSlab extends SlabBlock {
    ...
    public BlockState getPlacementState(ItemPlacementContext ctx) {
        if(!this.getDefaultState().canPlaceAt(ctx.getWorld(), ctx.getBlockPos())){
            return Block.pushEntitiesUpBeforeBlockChange(this.getDefaultState(), ModBlocks.DIRT.getSlabBlock().getPlacementState(ctx), ctx.getWorld(), ctx.getBlockPos());
        }
    ...
    public void scheduledTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        world.setBlockState(pos, pushEntitiesUpBeforeBlockChange(state, ModBlocks.DIRT.getSlabBlock().getDefaultState().with(TYPE, world.getBlockState(pos).get(TYPE)), world, pos));
    }
